...
- Branch protection requires `test` (starter) to pass, and the branch to be up to date with `main`, before merge is allowed
- **Squash merge only** — the PR title becomes the squash commit on `main`
- **PR titles are what release-please parses** on `main`; must be valid conventional commits. Use `feat!:` for breaking changes (triggers a major bump)
