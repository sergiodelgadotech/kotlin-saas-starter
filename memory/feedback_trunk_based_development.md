...
Both `kotlin-saas-starter` and `kotlin-saas-template` use trunk-based development. Commit directly to `main`. Do not create feature branches or git worktrees for plan execution, even when the default Claude Code/superpowers guidance says to.
...
- Note for this repo specifically: release-please opens its own release PR off `main` — that PR is part of the release flow, not a feature-branch workflow.
